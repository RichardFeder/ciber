#!/usr/bin/env python3
"""Compare CIBER x galaxy cross spectra between collaborator and local outputs.
...
def load_collab_flux(tm: int, zlo: float, zhi: float) -> np.ndarray:
    p = COLLAB_DIR / f"img_flux_band_ciber_{tm}_z_{zlo:.1f}_{zhi:.1f}.npy"
    return np.load(p)
...
def compute_cross_dell(flux_map: np.ndarray, overdens_map: np.ndarray, fov_deg: float, nbins: int = 20):
    pixsize_arcsec = fov_deg * 3600.0 / float(flux_map.shape[0])
    mI = flux_map - np.mean(flux_map)
    lb, cl, clerr = get_power_spec(mI, map_b=overdens_map, nbins=nbins, pixsize=pixsize_arcsec)
...
def plot_tm(tm: int):
    ...
    for i, (zlo, zhi) in enumerate(ZBINS):
        ...
        c_flux = load_collab_flux(tm, zlo, zhi)
        c_gal = load_collab_overdens_i(zlo, zhi)
        o_flux = load_our_flux(tm, zlo, zhi)
        o_gal = load_our_overdens(tm, zlo, zhi)

        lb_c, dell_c, derr_c = compute_cross_dell(c_flux, c_gal, fov_deg=10.0, nbins=20)
        lb_o, dell_o, derr_o = compute_cross_dell(o_flux, o_gal, fov_deg=8.0, nbins=20)
    ...
    axes[0].set_ylabel(r"$D_\ell^{Ig}$")
    axes[0].legend(fontsize=8)
    fig.suptitle(f"TM{tm}: Cross Spectrum Comparison (hsc_i) using get_power_spec", fontsize=12)
    fig.tight_layout(rect=[0, 0, 1, 0.92])

    fig_path = OUTDIR / f"TM{tm}_cross_compare_getpowerspec_hsc_i.png"
...
def main():
    ...
    txt = OUTDIR / "cross_compare_getpowerspec_hsc_i.txt"
