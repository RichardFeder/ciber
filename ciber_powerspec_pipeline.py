...
class CIBER_PS_pipeline():
	...
	def noise_model_realization(self, inst, maplist_split_shape, noise_model, fft_obj=None, adu_to_sb=True):
		
		chi2_realiz = (np.random.chisquare(2., size=maplist_split_shape)/2.83)**2
	...
	def compute_FW_from_noise_sim(self, ifield=None, inst=None, mask=None, apply_mask=True, noise_model=None, noise_model_fpath=None, verbose=False, inplace=True, \
		...
		if use_pyfftw:
			...
			for i in range(n_split):
				...
				rnmaps = self.noise_model_realization(inst, maplist_split_shape, noise_model, fft_obj=fft_objs[0])
...
#                 cl2d_all[i*nsims//n_split:(i+1)*nsims//n_split,:,:] = fftshift(fft_objs[1](simmaps).real)/self.cal_facs[inst]**2
		...
		if noise_model is None:
			if noise_fpath is None:
				if ifield is not None and inst is not None:
					...
					print('out of luck, need more information')
					return None
	...
	def monte_carlo_power_spectrum_sim(self, ifield, inst, image=None, mask=None, apply_mask=True, noise_model=None, masked_noise_model=None, noise_model_fpath=None,\
		...
		if apply_FW:
			if FW_image is None:
				...
				self.FW_image = masked_noisecl2d
			else:
...
#                 simmaps[j, :, :] = simmap
	...
	def compute_shot_noise_map(self, inst, image, shot_sigma=None, nfr=10, frame_rate=1):
		
		# convert to e-/s
		
		if shot_sigma is None:
			flight_signal = image*self.g1_facs[inst]
		
			# make shot noise term from flight data Garnett and Forrest
		
			shot_sigma = np.sqrt((flight_signal/(nfr/frame_rate))*((nfr**2+1)/(nfr**2 - 1)))
		
		shotmap_eps = np.random.normal(0, 1, size=image.shape)*shot_sigma
		shotmap_adufr = shotmap_eps/self.g1_facs[inst]
		
		return shotmap_adufr
	...
	def compute_processed_power_spectrum(self, inst, mask=None, apply_mask=True, N_ell=None, B_ell=None, inv_Mkk=None,\
		...
		if apply_mask:
			...
			if verbose:
				print('No masking, subtracting image by its mean..')
		...
		if noise_debias: # subtract noise bias
			...
			if verbose:
				print('after noise subtraction, ', cl_proc)
		...
		if mkk_correct: # apply inverse mode coupling matrix to masked power spectrum
			...
			if verbose:
				print('after mkk, cl_proc is ', cl_proc)
