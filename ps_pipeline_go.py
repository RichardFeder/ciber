...
def run_cbps_pipeline(cbps, inst, nsims, run_name, ifield_list = None, ps_type='auto', cross_type='ciber', cross_inst=2, cross_gal=None, \
                            simidx0=0, datestr='100921', datestr_trilegal=None, data_type='mock',\
                            ...
                            mask_base_path=None, mkk_base_path=None, bls_base_path=None, masking_maglim=17.5, isl_rms_fpath=None, bls_fpath=None, n_cib_set=20, \
                            mask_tail='abc110821', n_FW_sims=500, n_FW_split=10, \
                            ff_fpath='data/flatfield/TM1_FF/TM1_field4_mag=17.5_FF.fits', \
                            zl_levels=None, ff_biases=None, save=True, verbose=True, **kwargs):
    ...
    pscb_dict = dict({'apply_mask':True, 'with_inst_noise':True, 'with_photon_noise':True, 'apply_FW':True, \
                    ...
                    'load_trilegal':True , 'subtract_subthresh_stars':True, 'ff_bias_correct':True, 'save_ff_ests':True, 'plot_maps':True, \
                     'draw_cib_setidxs':False, 'aug_rotate':False, 'load_noise_bias':False, 'transfer_function_correct':False, 'save_intermediate_cls':True})

    float_param_dict = dict({'ff_min':0.5, 'ff_max':1.5, 'clip_sigma':5, 'ff_stack_min':2, 'nmc_ff':10, \
                      'theta_mag':0.01, 'niter':10, 'dgl_scale_fac':5, 'smooth_sigma':5, 'indiv_ifield':6, 'nfr_same':25})
    ...
    trilegal_base_path = ciber_mock_fpath+datestr_trilegal+'/'

    if mask_base_path is None:
        mask_base_path = base_path+'TM'+str(inst)+'/masks/'
    if mkk_base_path is None:
        mkk_base_path = base_path+'TM'+str(inst)+'/mkk/'
    if bls_base_path is None:
        bls_base_path = base_path+'TM'+str(inst)+'/beam_correction/'
    ...
    if isl_rms_fpath is not None:
        mean_isl_rms = load_isl_rms(isl_rms_fpath, masking_maglim, nfield)
...
#         diff_realizations = np.zeros(field_set_shape)
        ...
        if data_type=='mock':
            ...
            test_set_fpath = ciber_mock_fpath+datestr+'/TM'+str(inst)+'/cib_with_tracer_5field_set'+str(cib_setidx)+'_'+datestr+'_TM'+str(inst)+'.fits'
            ...
            joint_masks, observed_ims, total_signals,\
                    ...
                    ff_truth, diff_realizations, zl_perfield = generate_synthetic_mock_test_set(cbps, inst, ifield_list,\
                                                        test_set_fpath=test_set_fpath, load_ptsrc_cib=pscb_dict['load_ptsrc_cib'], \
                                                        mock_trilegal_path=trilegal_fpath, with_inst_noise=pscb_dict['with_inst_noise'], noise_models=read_noise_models, \
                                                        with_photon_noise=pscb_dict['with_photon_noise'], ff_truth=smooth_ff, apply_flat_field=pscb_dict['apply_smooth_FF'], \
                                                        indiv_ifield=float_param_dict['indiv_ifield'], dgl_scale_fac = float_param_dict['dgl_scale_fac'], \
                                                                                show_plots=pscb_dict['plot_maps'])
            
            
            if i==0 and pscb_dict['compute_beam_correction']:
                
                if bls_fpath is None:
                    bl_est_path = bls_base_path+'ptsrc_blest_TM'+str(inst)+'_simidx'+str(cib_setidx)+'.npz'
                    print('Computing beam correction to be saved to bls_fpath='+bl_est_path+'..')
        
                lb, diff_norm_array,\
                        cls_masked_tot, cls_masked_tot_bright = estimate_b_ell_from_maps(cbps, inst, ifield_list, observed_ims, simidx=i, pixsize=7., J_bright_mag=11., J_tot_mag=17.5, plot=False, save=False, \
                                            niter = float_param_dict['niter'], ff_stack_min=float_param_dict['ff_stack_min'], data_type='mock', mask_base_path=mask_base_path, ell_norm=5000)
                    
                B_ells = np.sqrt(diff_norm_array).copy()
                print('B_ells is ', B_ells)

                np.savez(bl_est_path, \
                    lb=lb, J_bright_mag=11, J_tot_mag=17.5,\
            ...
            for fieldidx, ifield in enumerate(ifield_list):
                ...
                if data_type=='observed':
                    ...
                    if pscb_dict['load_ptsrc_cib']:
                        print('mask path is ', mask_base_path+'joint_mask_ifield'+str(ifield)+'_inst'+str(inst)+'_simidx'+str(i)+'_'+mask_tail+'.fits')
                        joint_maskos[fieldidx] = fits.open(mask_base_path+'joint_mask_ifield'+str(ifield)+'_inst'+str(inst)+'_simidx'+str(i)+'_'+mask_tail+'.fits')['joint_mask_'+str(ifield)].data
#                         joint_maskos[fieldidx] = fits.open(base_path+'TM'+str(inst)+'/masks/ff_joint_masks/joint_mask_with_ffmask_ifield'+str(ifield)+'_inst'+str(inst)+'_simidx'+str(i)+'_'+mask_tail+'.fits')['joint_mask_'+str(ifield)].data
                        inv_Mkks.append(fits.open(mkk_base_path+'mkk_estimate_ifield'+str(ifield)+'_inst'+str(inst)+'_simidx'+str(i)+'_'+mask_tail+'.fits')['inv_Mkk_'+str(ifield)].data)
                    else:
                        maskidx = i%10
        ...
        if pscb_dict['save_intermediate_cls']:
            cls_masked_prenl, cls_postffb_corr, cls_masked_postnl,\
                 cls_postmkk_prebl, cls_postbl, cls_postffb_corr, cls_post_isl_sub = [np.zeros(single_ps_set_shape) for x in range(7)]
        ...
        for fieldidx, obs in enumerate(observed_ims):
            ...
            if i > 0 or pscb_dict['load_noise_bias']:
                ...
                fourier_weights_nofluc, mean_cl2d_nofluc = cbps.estimate_noise_power_spectrum(nsims=n_FW_sims, n_split=n_FW_split, apply_mask=pscb_dict['apply_mask'], \
      ...
      #       	# print('to ', float(np.sum(target_mask))/float(cbps.dimx**2))
            ...
            lb, processed_ps_nf, cl_proc_err, _ = cbps.compute_processed_power_spectrum(inst, apply_mask=pscb_dict['apply_mask'], \
                                            ...
                                            apply_FW=pscb_dict['apply_FW'], verbose=True, noise_debias=True, \
            ...
            if data_type=='mock':
                if pscb_dict['load_ptsrc_cib']: # false for observed
                    ...
                    cib_cl_file = fits.open(base_path+'TM'+str(inst)+'/powerspec/cls_cib_vs_maglim_ifield'+str(ifield_list[fieldidx])+'_inst'+str(inst)+'_simidx'+str(cib_setidx)+'_Vega_magcut.fits')
            ...
            if pscb_dict['subtract_subthresh_stars']:
                ...
                trilegal_cl_fpath = trilegal_base_path+'TM'+str(inst)+'/powerspec/cls_trilegal_vs_maglim_ifield'+str(ifield_list[fieldidx])+'_inst'+str(inst)+'_simidx'+str(cib_setidx)+'_Vega_magcut.fits'
