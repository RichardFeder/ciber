...
def decision_tree_train(training_catalog, feature_names, feature_bands=None, test_catalog=None, outlablstr='j_mag_best', J_mag_lim=18.5, max_depth=5, show_tree=False):
    ...
    if show_tree:
        ...
        if feature_bands is None:
            ...
            pass
        dt_fig = plot_decision_tree(fig, feature_bands, class_names=[above_label, below_label], max_depth=max_depth)

    predictions_train = fig.predict(train_features)
    ...
    if test_catalog is not None:
        ...
        predictions_test = fig.predict(test_features)
