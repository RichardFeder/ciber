...
def make_binned_coverage_mask_and_mkk(posx, posy, catalog_mode, nx=None, ny=None, n_mkk_sim=100, n_split=2, \
    ...
    if nreg is None:
        nreg = nx+1
#     H, xedges, yedges = np.histogram2d(posx, posy, bins=[np.linspace(0, nx, nx+1), np.linspace(0, ny, nreg)])
    ...
    print('posx lenght ', len(posx))
    print('coverage mask full res ', np.sum(coverage_mask_fullres)/float(nx*ny))
    ...
    if save:
        ...
        inv_Mkk = save_mkks(save_fpath, av_Mkk=av_Mkk, return_inv_Mkk=True, mask=coverage_mask, \
...
def preprocess_c15_catalog(catalog_fpath=None, ramin=149.4, ramax=150.8, decmin=1.5, decmax=2.9, save=False, \
    ...
    c15_catalog = np.array([c15_ra, c15_dec, c15_x, c15_y, c15_J_AB, \
                           c15_H_AB, c15_CH1_AB, c15_CH2_AB]).transpose()
    ...
    full_df = pd.DataFrame({'x':c15_x_sel, 'y':c15_y_sel, 'J': c15[:,4], 'H':c15[:,5],\
                   'mag_CH1':c15[:,6], 'mag_CH2':c15[:,7]})
    
    if save:
        save_fpath = config.ciber_basepath+'data/catalogs/COSMOS15/COSMOS15_hjmcc_JH_CH1CH2_AB.csv'
        print('Saving catalog to ', save_fpath)
        full_df.to_csv(save_fpath)
    ...
    if binned_coverage_mask_mkk:
        av_Mkk, inv_Mkk, coverage_mask, save_fpath = make_binned_coverage_mask_and_mkk(c15_x_sel, c15_y_sel, 'COSMOS15',\
                                                                               nx=sqdim, ny=sqdim, addstr='test', save=save, nreg=nreg)
...
def simulate_deep_cats_correlation_coeff_cosmos(m_min_J_list, m_min_H_list, inst=1, ifield_choose = 4, include_IRAC_mask=False, maglim_IRAC=18., m_max=28, \
                                               ...
                                               cl_pred_basepath=config.ciber_basepath+'data/cl_predictions/', add_str=None):
...
#         plt.savefig('/Users/richardfeder/Downloads/cosmos2020_Jlt'+str(m_min_J_list[magidx])+'_Hlt'+str(m_min_H_list[magidx])+'_081423.png', bbox_inches='tight')
   	...
   	if save:
   		cl_save_fpath = cl_pred_basepath+'cosmos20_cl_prediction_vs_mag_auto_cross_JH_CH1CH2_mask'
   		if add_str is not None:
   			cl_save_fpath += '_'+add_str
   		...
   		np.savez(cl_save_fpath+'.npz', lb=lb, \
	        all_clauto_J=all_clauto_J, all_clauto_H=all_clauto_H, all_clx_JH=all_clx_JH, all_clauto_CH1=cl_autos[2], \
	        all_clauto_CH2=cl_autos[3], all_clx_J_CH1=cl_crosses[1], all_clx_J_CH2=cl_crosses[2], all_clx_H_CH1=cl_crosses[3], \
	        all_clx_H_CH2=cl_crosses[4], all_clx_CH1_CH2=cl_crosses[5])
